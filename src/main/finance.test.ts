...
describe('Б8: parseCsvLine — кавычки с разделителем внутри', () => {
  it('поле в кавычках с точкой с запятой не режется', () => {
    expect(parseCsvLine('01.06.2026;"Магазин; продукты";1500', ';')).toEqual(['01.06.2026', 'Магазин; продукты', '1500'])
  })
  it('экранированная кавычка внутри поля', () => {
    expect(parseCsvLine('"ООО ""Ромашка""";500', ';')).toEqual(['ООО "Ромашка"', '500'])
  })
...
describe('Этап 7.3: budgetCarryover — перенос остатка бюджета', () => {
  it('без переноса — ноль', () => {
    expect(budgetCarryover(10000, 3000, false)).toBe(0)
  })
  it('перенос неистраченного остатка', () => {
    expect(budgetCarryover(10000, 3000, true)).toBe(7000)
  })
  it('перерасход не даёт отрицательного переноса', () => {
    expect(budgetCarryover(10000, 12000, true)).toBe(0)
  })
})
